...
--DBMS_STATS.GATHER_TABLE_STATS(ownname => 'SIP', tabname => 'TB_WTMPS_SESSIONS_ARCHIVE', partname => part, cascade => TRUE, ESTIMATE_PERCENT => NULL, granularity => 'PARTITION',DEGREE=>8);


insert /*+ APPEND */ into TB_WTMPS_SESSIONS_ARCHIVE@SIPARCH 
select * from SIP.TB_WTMPS_SESSIONS_ARCHIVE where
...
--DBMS_STATS.GATHER_TABLE_STATS(ownname => 'SIP', tabname => 'TB_WTMPS_ARCHIVE', partname => part, cascade => TRUE, ESTIMATE_PERCENT => NULL, granularity => 'PARTITION',DEGREE=>8);
insert /*+ APPEND */ into TB_WTMPS_ARCHIVE@SIPARCH 
select * from SIP.TB_WTMPS_ARCHIVE where
...
--DBMS_STATS.GATHER_TABLE_STATS(ownname => 'SIP', tabname => 'TB_WTMPS_SESSION_CHARGES', partname => part, cascade => TRUE, ESTIMATE_PERCENT => NULL, granularity => 'PARTITION',DEGREE=>8);
insert /*+ APPEND */ into TB_WTMPS_SESSION_CHARGES@SIPARCH 
select * from SIP.TB_WTMPS_SESSION_CHARGES where
