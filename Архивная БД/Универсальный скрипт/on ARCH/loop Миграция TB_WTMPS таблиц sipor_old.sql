...
--Проверка
    ...
    /*
select num_rows into numrows_arch from SIP_OR.PART_VIEW where table_name='TB_WTMPS_SESSIONS_ARCHIVE' and partition_name=part; --кол-во в архивной бд
    if (numrows_arch > 0)  then
    raise_application_error( -20001, 'Записи в SIPARCH TB_WTMPS_SESSIONS_ARCHIVE:'||part||' уже присутсвуют' );
    end if;
*/
...
insert /*+ APPEND */ into SIP_OR.TB_WTMPS_SESSIONS_ARCHIVE 
select * from TB_WTMPS_SESSIONS_ARCHIVE@SIPOR_OLD where
...
--Проверка
/*
select num_rows into numrows_arch from SIP_OR.PART_VIEW where table_name='TB_WTMPS_ARCHIVE' and partition_name=part; --кол-во в архивной бд

    if (numrows_arch > 0)  then
    raise_application_error( -20001, 'Записи в SIPARCH TB_WTMPS_ARCHIVE:'||part||' уже присутсвуют' );
    end if;
*/    
...
insert /*+ APPEND */ into SIP_OR.TB_WTMPS_ARCHIVE 
select * from TB_WTMPS_ARCHIVE@SIPOR_OLD where
...
--Проверка
/*
select num_rows into numrows_arch from SIP_OR.PART_VIEW where table_name='TB_WTMPS_SESSION_CHARGES' and partition_name=part; --кол-во в архивной бд

    if (numrows_arch > 0)  then
    raise_application_error( -20001, 'Записи в SIPARCH TB_WTMPS_SESSION_CHARGES:'||part||' уже присутсвуют' );
    end if;
    */
...
insert /*+ APPEND */ into SIP_OR.TB_WTMPS_SESSION_CHARGES 
select * from TB_WTMPS_SESSION_CHARGES@SIPOR_OLD where
...
commit;
execute immediate 'ALTER SESSION CLOSE DATABASE LINK SIPOR_OLD'; --1 раз или 4 ?
