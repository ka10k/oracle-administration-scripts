...
select 'PSANNIKOV' usr from dual
)
, tbrep (usr, gr, r) as (
...
--select * from STMT_AUDIT_OPTION_MAP where name in (select PRIVILEGE from (


--++AUDIT SESSION;
set linesize 2000;
select 'AUDIT ' || LISTAGG(name, ',') WITHIN GROUP (ORDER BY 1) || ' BY PSANNIKOV BY ACCESS' AS names   from STMT_AUDIT_OPTION_MAP where name in (select PRIVILEGE from (
